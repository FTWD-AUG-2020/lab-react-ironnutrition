...
function App() {
  ...
  const searchFood = (e) => {
    ...
    let result = [...allFoods].filter(food => food.name.toLowerCase().includes(e.target.value))
    setFoods(result)
  }
  ...
  return (
    <div className="App">
      ...
      <input onChange={searchFood} type="text" name="search" placeholder="Search" style={{width: '100%'}} />
      {showForm()}
      {showFoods()}
